    ...
    for episode in range(num_episodes):
        ...
        while nstep_rewards or not done:
            ...
            if not done:
                ...
                #print(f"Done: {done}") if debug else 0

                # Keep accumulating until there's enough for the first n-step update
                if len(nstep_rewards) < n:
                    print(f"Step: {step} | State: {old_state} | Action: {delta} | Next State: {state} | Reward: {reward} | State Values: {v}") if debug else 0
                    continue
            ...
            #print(f"nstep_states: {nstep_states}") if debug else 0
            #print(f"alpha: {alpha}") if debug else 0
